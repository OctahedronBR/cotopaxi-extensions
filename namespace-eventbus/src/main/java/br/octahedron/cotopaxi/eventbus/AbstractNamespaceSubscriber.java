...
public abstract class AbstractNamespaceSubscriber implements Subscriber {
	...
	private static final Log logger = new Log(AbstractNamespaceSubscriber.class);
	...
	public final void eventPublished(Event event) {
		try {
			if (event instanceof NamespaceEvent) {
				String ns = ((NamespaceEvent)event).namespace();
				logger.info("Processing event %s for namespace %s", event.getClass(), ns);
				namespaceManager.changeToNamespace(ns);
			}
